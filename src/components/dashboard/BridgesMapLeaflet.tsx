...
function MapBounds({ bridges }: { bridges: Bridge[] }) {
  ...
  useEffect(() => {
    ...
    if (validBridges.length === 0) return;

    const bounds = L.latLngBounds(validBridges.map((b) => [b.coordinates!.lat, b.coordinates!.lng]));
  ...
  return (
    <div className="rounded-lg border bg-card overflow-hidden">
      ...
      <div className="h-[450px] w-full">
        <MapContainer center={defaultCenter} zoom={4} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
          <TileLayer
                ...
                <div className="p-1">
                  {bridge.image && (
                    <div className="mb-2 rounded overflow-hidden h-24">
                      <img src={bridge.image} alt={bridge.name} className="w-full h-full object-cover" />
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Button size="sm" className="w-full mt-2" onClick={() => navigate(`/bridge/${bridge.id}`)}>
